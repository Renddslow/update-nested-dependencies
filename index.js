...
module.exports = (packages = []) => {
	...
	function traverse(tree) {
		...
		const updatedPackages = packageKeys.filter((p) => newTree[p].hasNext || newTree[p].updatedInternally);
		const packagesNeedingVisitation = packageKeys.filter((p) => !newTree[p].hasNext && !newTree[p].updatedInternally);
